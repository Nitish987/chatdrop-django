...
from .models import User, PreKeyBundle, ProfilePhoto, ProfileCoverPhoto
...
# user prekey bundle serializer
class UserPreKeyBundleSerializer(serializers.ModelSerializer):
    class Meta:
        model = PreKeyBundle
        fields = ['reg_id', 'device_id', 'prekeys', 'signed_prekey', 'identity_key']

    def validate(self, attrs):
        reg_id = attrs.get('reg_id')
        device_id = attrs.get('device_id')
        prekeys = attrs.get('prekeys')
        signed_prekey = attrs.get('signed_prekey')
        identity_key = attrs.get('identity_key')

        if validators.is_empty(reg_id) or validators.is_empty(device_id) or validators.is_empty(prekeys) or validators.is_empty(signed_prekey) or validators.is_empty(identity_key):
            raise serializers.ValidationError({'preKeyBundle': 'Invalid PreKey Bundle.'})
...
class ProfileCoverPhotoUpdateSerializer(serializers.ModelSerializer):
    cover_photo = serializers.ImageField()

    class Meta:
        model = ProfileCoverPhoto
        fields = ['cover_photo']
    ...
    def validate(self, attrs):
        ...
        if cover_photo == None:
            raise serializers.ValidationError({'user': 'Invalid Profile Cover Pic.'})

import user_agents
from rest_framework import authentication
from rest_framework import exceptions
from .models import User
from .jwt_token import Jwt, EncryptedJwt
from .services import LoginStateTokenService, WebLoginStateTokenService
from channels.middleware import BaseMiddleware
from channels.db import database_sync_to_async
from urllib.parse import parse_qs


# User Authentication
class UserAuthentication(authentication.BaseAuthentication):
    def authenticate(self, request):
        try:
            # getting validation success and payload from authentication token
            success, payload = Jwt.validate(request.META.get('HTTP_AUTHORIZATION'))
            payload_data = payload['data']
        
            # validating authentication token
            if not success or payload['type'] != 'LI' or payload_data['uid'] != request.META.get("HTTP_UID"):
                return None

            try:
                # fetching user from database
                user = User.objects.get(uid=payload_data['uid'])
            except User.DoesNotExist:
                raise exceptions.AuthenticationFailed('No such Account found.')

            try:
                login_state = LoginStateTokenService.get(user=user)

                if login_state['login_token'] != request.META['HTTP_LST']:
                    return None

                # passing user agent data
                ua = user_agents.parse(request.META['HTTP_USER_AGENT'])

                # validing login state token data with user agent
                if login_state['browser'] != ua.browser.family or login_state['device'] != ua.device.family or login_state['os'] != ua.os.family:
                    return None
            except:
                raise exceptions.AuthenticationFailed('No Login user found.')

            return (user, None)
        except:
            return None
        


# User Web Authentication
class UserWebAuthentication(authentication.BaseAuthentication):
    def authenticate(self, request):
        try:
            # getting validation success and payload from authentication token
            success, payload = EncryptedJwt.validate(request.COOKIES.get('at'))
            payload_data = payload['data']
        
            # validating authentication token
            if not success or payload['type'] != 'LI' or payload_data['uid'] != request.META.get("HTTP_UID"):
                return None

            try:
                # fetching user from database
                user = User.objects.get(uid=payload_data['uid'])
            except User.DoesNotExist:
                raise exceptions.AuthenticationFailed('No such Account found.')

            try:
                login_state = WebLoginStateTokenService.get(user=user, token=request.META['HTTP_LST'])

                # passing user agent data
                ua = user_agents.parse(request.META['HTTP_USER_AGENT'])

                # validing login state token data with user agent
                if login_state.browser != ua.browser.family or login_state.device != ua.device.family or login_state.os != ua.os.family:
                    return None
            except:
                raise exceptions.AuthenticationFailed('No Login user found.')

            return (user, None)
        except:
            return None



# Websocket User Authentication
class WebSocketUserAuthenticationMiddleware(BaseMiddleware):
    def __init__(self, inner):
        super().__init__(inner)

    @database_sync_to_async
    def authenticate(self, wat, lst):
        try:
            success, payload = Jwt.validate(wat)
            payload_data = payload['data']
        
            # validating authentication token
            if not success or payload['type'] != 'WSLI':
                return None

            try:
                # fetching user from database
                user = User.objects.get(uid=payload_data['uid'])
            except User.DoesNotExist:
                raise exceptions.AuthenticationFailed('No such Account found.')

            try:
                login_state = LoginStateTokenService.get(user=user)
                if login_state['login_token'] != lst:
                    return None
                
                if not login_state:
                    return None
            except:
                raise exceptions.AuthenticationFailed('No Login user found.')

            return user
        except:
            return None

    async def __call__(self, scope, receive, send):
        try:
            params = parse_qs(scope["query_string"].decode('utf-8'))
            # setting user if authentication successfull
            scope['user'] = await self.authenticate(params["wat"][0], params["lst"][0])
        except:
            # setting user if authentication successfull
            scope['user'] = None

        return await super().__call__(scope, receive, send)

